...
static const unsigned short *codepage_table = _codepage_table;
static const unsigned short *codepage_extras = _codepage_extras;
 ...
 *  encoding format.
 */
void register_uformat(int type, int (*ugetc)(const char *s), int (*ugetx)(char **s), int (*usetc)(char *s, int c), int (*uwidth)(const char *s), int (*ucwidth)(int c), int (*uisok)(int c), int uwidth_max)
{
...
static int sprint_char(STRING_ARG *string_arg, SPRINT_INFO *info, long val)
{
   ...
   string_arg->data = malloc((MAX(1, info->field_width) * uwidth_max(U_CURRENT) + ucwidth(0)) * sizeof(char));
 ...
 */
static int sprint_string(STRING_ARG *string_arg, SPRINT_INFO *info, const char *s)
{
   int pos = 0, len = 0;
   int c;
 ...
 */
static int decode_format_string(char *buf, STRING_ARG *string_arg, const char *format, va_list args)
{
   SPRINT_INFO info;
   int *pstr_pos;
